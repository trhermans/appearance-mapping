// Revised from svm-predict.c for fast histogram intersection kernel
...
void predict(FILE *input, FILE *output,Array2dC<double>& eval,const int m,const int upper_bound)
{
	...
	while(readline(input) != NULL)
	{
		...
		{
			predict_label = hik_predict_probability(model,eval,m,upper_bound,x,prob_estimates);
		...
		{
			predict_label = hik_predict(model,eval,m,upper_bound,x);
			fprintf(output,"%g\n",predict_label);
		}
...
{
	printf(
	"Usage: hik-predict -D dimension -u upper_bound [options] test_file model_file output_file\n"
	"options:\n"
...
int main(int argc, char **argv)
{
    ...
    UseSVM_Histogram_FastEvaluationStructure(argv[i+1],m,upper_bound,eval,&model,false);
    ...
    {
        fprintf(stderr,"hik-predict only support histogram intersection kernel (-t 5)\n");
        exit(-1);
    }
