...
public:
    ...
    // If you do not want to save the clustered model to disk,
    // just set "modelname=NULL"
    // Perform histogram intersection kernel clustering on integer input data
    void HIK_Clustering(Array2d<int>& features, const int _k,
                        const int max_iteration, const int _upper_bound,
                        const bool oneclass_svm, const bool verbose,
                        const char* modelname);
    // Perform k-means or k-median clustering on real valued input data
    void Kmeans_Clustering(Array2d<double>& features,const int _k,
                           const int max_iteration, const bool useMedian,
                           const bool oneclass_svm, const bool verbose,
                           const char* modelname);

    // Perform histogram intersection kernel clustering on integer input data
    void HIK_Clustering(const char* filename,const int _k,
                        const int max_iteration, const int _upper_bound,
                        const bool oneclass_svm, const bool verbose,
                        const char* modelname);
    // Perform k-means or k-median clustering on real valued input data
    void Kmeans_Clustering(const char* filename, const int _k,
                           const int max_iteration, const bool useMedian,
                           const bool oneclass_svm, const bool verbose,
                           const char* modelname);
private:
