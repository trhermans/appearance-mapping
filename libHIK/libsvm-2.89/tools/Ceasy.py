...
cmd = '%s -svmtrain "%s" -log2g 1,1,1 -t %s -gnuplot "%s" "%s"' % (grid_py, svmtrain_exe, t, gnuplot_exe, scaled_file)
...
# Note that "-t %g" is added by Jianxin Wu
cmd = '%s -c %s -t %s -g %s "%s" "%s"' % (svmtrain_exe,c,t,g,scaled_file,model_file)
print 'Training...'
Popen(cmd, shell = True, stdout = PIPE).communicate()
